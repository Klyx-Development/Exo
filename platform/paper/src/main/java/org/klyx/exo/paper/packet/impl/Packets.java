package org.klyx.exo.paper.packet.impl;
...
import org.klyx.exo.paper.packet.PacketHelper;
import org.klyx.exo.paper.packet.impl.handlers.PacketHandlerUnregisterer;
import org.klyx.exo.paper.packet.impl.handlers.PacketHandlers;
import org.klyx.exo.paper.packet.impl.handlers.PlayerConnectionHandler;
import org.klyx.exo.paper.packet.impl.handlers.PriorityPacketHandler;
...
public class Packets {
                        ...
                        for (@Nullable Packet<?> nested : bundlePacket.subPackets()) {
                            ...
                            if (hasHandlers(nested)) {
                                if (checkedClasses.contains(nestedClass)) {
                                    ...
                                    ExoPaper.logger().warn("Skipping packet processing for " + nestedClass.getName() + " to avoid infinite loop");
                                }
                    ...
                    for (PriorityPacketHandler<?, ?> handler : handlers) {
                        ...
                        while (index < packets.size()) {
                            ...
                            try {
                                ...
                                ExoPaper.logger().error("Error in packet handler", e);
    ...
    public void sendPackets(UUID uuid, Packet<?>... packets) {
        getPlayer(uuid).ifPresentOrElse(player -> sendPackets(player, packets), () -> ExoPaper.logger().warn("Player " + uuid + " is not online"));
    }
